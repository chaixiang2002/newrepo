...
//8.
void CreateGraph(ListGraph *G,Elemtype Vertices_F[],int num_Vertices_F,RowCol Edges_F[],int num_Edges_F){
    int i,k;
    AdjInit(G);
    for(i=0;i<num_Vertices_F;i++)
    InsertVertex(G,i,Vertices_F);
    for(i=0;i<num_Edges_F;i++)
    InsertEdge(G,Edges_F[k].col);
